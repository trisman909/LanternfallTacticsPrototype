    ...
    public static class BuildPrototype
    {
        ...
        static void PatchWebGLForResponsivePreview(string path)
        {
            ...
            if(File.Exists(css))
            {
                ...
                text += "#lanternfall-build-proof { position: fixed; z-index: 100000; left: max(6px, env(safe-area-inset-left)); top: max(4px, env(safe-area-inset-top)); padding: 3px 8px; border: 1px solid #b9832d; border-radius: 4px; background: rgba(0,0,0,.88); color: #ffd36f; font: 700 13px/16px Arial, sans-serif; pointer-events: none; }\n";
