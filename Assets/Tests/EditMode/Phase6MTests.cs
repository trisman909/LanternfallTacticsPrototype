...
namespace Lanternfall.Tests
{
    public sealed class Phase6MTests
    {
        ...
        {
            var layout=MobileLayout.Compute(width,height);var hud=CombatHudLayout.Compute(layout.Panel,layout.Portrait,layout.CompactLandscape);Assert.True(layout.PhoneLandscape);Assert.GreaterOrEqual(hud.HazardNote.height,44f);Assert.True(hud.SkillCards.All(r=>r.height>=60f));Assert.GreaterOrEqual(hud.EndTurnButton.height,56f);Assert.True(hud.RequiredElementsFit(layout.Panel));Assert.False(hud.HasEssentialOverlap());Assert.False(hud.HazardNote.Overlaps(hud.SkillCards[0]));Assert.False(hud.HazardNote.Overlaps(hud.EndTurnButton));
        }
