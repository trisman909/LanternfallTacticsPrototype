...
namespace Lanternfall.Tests
{
    public sealed class Phase6MTests
    {
        ...
        {
            var layout=MobileLayout.Compute(width,height);Assert.True(layout.PhoneLandscape);Assert.GreaterOrEqual(layout.ThreatPanel.width,210f);Assert.True(layout.SkillButtons.All(r=>r.height>=56f));Assert.GreaterOrEqual(layout.ActionButton.height,56f);Assert.LessOrEqual(layout.Board.xMax,layout.ThreatPanel.xMin);Assert.LessOrEqual(layout.Board.yMax,layout.SkillBar.yMin);
        }
